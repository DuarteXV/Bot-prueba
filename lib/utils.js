...
export function channelContext(overrides = {}) {
  ...
  // Thumb: prioridad overrides → config.channelThumb (buffer cargado al conectar)
  let thumbnail
  if (overrides.thumbnail) {
    thumbnail = overrides.thumbnail
  } else if (config.channelThumb) {
    thumbnail = Buffer.from(config.channelThumb, 'base64')
  }

  return {
    contextInfo: {
      externalAdReply: {
        title: overrides.title || config.botName,
        body: overrides.body || config.channelName,
        thumbnailUrl: overrides.thumbnailUrl || '',
        thumbnail,
        sourceUrl: overrides.sourceUrl || config.channelInviteLink,
        mediaType: 1,
